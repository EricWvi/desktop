...
`ora-logging` supplies the bridge. `OraGitlancerLogger` projects `log_command` to an `ora_info!` event containing only the alphabetic Git subcommand as `operation`; it deliberately drops absolute cwd and all arguments so commit messages, paths, remotes, and credentials cannot become ordinary structured fields. `log_result` emits `ora_info!` on success or `ora_error!` on failure, carrying only `duration_ms` and `exit_code`. `register_gitlancer_logger()` constructs and registers it in one call.
